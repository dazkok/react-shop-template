...
const AlertComponent = (props: { message: string, alert_type: string }) => {
    if (props.alert_type === 'success') {
        return (
            <div className={'global-alert global-alert-success d-flex align-items-start'} role={'alert'}>
                <IconInfoSquareRoundedFilled />&nbsp;&nbsp;{props.message}
            </div>
        );
    } else if (props.alert_type === 'danger') {
        return (
            <div className={'global-alert global-alert-danger d-flex align-items-start'} role={'alert'}>
